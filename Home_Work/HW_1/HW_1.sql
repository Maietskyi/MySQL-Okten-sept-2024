...
select avg(price) as averagePriceKIA from cars group by model like 'KIA';
# знайти середнє арифметичне цін для кожної марки машин окремо
select avg(price) as averagePrice, model from cars group by model;
# підрахувати кількість кожної марки
select count(*) count, model from cars group by model;
# знайти марку машин кількість яких найбільше
select count(*) count, model from cars group by model order by count desc limit 1;
#
